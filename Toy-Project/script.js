...
document.addEventListener('DOMContentLoaded', async function () {
  const form = document.getElementById('guestbook-form'); // 입력폼
  const guestbookEntries = document.getElementById('guestbook-entries');
  const BASE_URL = 'http://3.38.74.127:8000/guestbook/'; // 백엔드 API 주소

  // 방명록 불러오기
  async function fetchGuestbooks() {
    const res = await fetch(BASE_URL); // GET 요청으로 방명록 데이터 가져오기
    const json = await res.json();
    // 최신순으로 보이기 위해 역순 정렬 -> 각 항목 추가
    if (json.status === 200) {
      ...
      json.data.reverse().forEach(entry => {
        ...
        guestbookEntries.appendChild(item); // 생성한 항목을 화면에 추가
      });
    } 
    else {
